...
class AlpacaNewsStreamSource(StatelessSource):
    ...
    def next(self):
        ...
        """
        logger.debug("Waiting for next message from Alpaca API")
        message = self._alpaca_client.recv()
        logger.info(f"Received message from Alpaca API: {message}")
        return message
...
class AlpacaNewsStreamClient:
    ...
    * Source of implementation inspiration: https://github.com/alpacahq/alpaca-py/blob/master/alpaca/common/websocket.py
    ...
    def _connect(self):
        ...
        """
        self._ws = create_connection(self.NEWS_URL,sslopt={"cert_reqs":ssl.CERT_NONE})
    ...
    def recv(self) -> Union[dict, List[dict]]:
        ...
        if self._ws:
            ...
            logger.info(f"[AlpacaNewsStream]: Parsed message: {json.dumps(message, indent=2)}")
